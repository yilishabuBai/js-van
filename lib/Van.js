...
function set($key, $value, $options) {
    const options = Object.assign(new VanOptions_1.default(VanType_1.default.localStorage), $options);
    for (const key in VanType_1.default) {
        if (isNaN(parseInt(key))) {
            const type = VanType_1.default[key];
...
function get($key, $options) {
    const options = $options || new VanOptions_1.default(VanType_1.default.localStorage);
    for (const key in VanType_1.default) {
        if (isNaN(parseInt(key))) {
            const type = VanType_1.default[key];
            if (type && options.type) {
